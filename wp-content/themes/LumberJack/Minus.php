...
<?
	...
	$bool=false;
		...
		$temp=$_SESSION["Mass"][$i];		
			if($temp["ID"]==$_POST["ID"])
			{
				if(intval($_SESSION["Mass"][$i]["kolichestvo"])>1)
		$_SESSION["Mass"][$i]["kolichestvo"]--;
  $bool=true;
			}
		}
			if($bool==false)
			{
	$mass=array("ID"=>$_POST["ID"],"Title"=>$_POST["Title"],"Price"=>$_POST["Price"],"Image"=>$_POST["Image"],"kolichestvo"=>1);
	array_push($_SESSION["Mass"],$mass);
			}
	}
	else
	{
	$mass=array(array("ID"=>$_POST["ID"],"Title"=>$_POST["Title"],"Price"=>$_POST["Price"],"Image"=>$_POST["Image"],"kolichestvo"=>1));
	$_SESSION["Mass"]=$mass;
	}
